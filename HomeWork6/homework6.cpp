...
//Raytracing of triangles with camera movement
#include <fstream>
#include <cmath>
#include <vector>
#include <string>
...
void render(const string & fileName, triangle * triangles, int sizeOfTriangles, Camera& camera) {
...
void animate(const unsigned int& frames, Camera& camera, int sizeOfTriangles, triangle * triangles){
	...
	const string frameName = "animation/frame_";
	
	for(int frame=0; frame<frames; frame++){
		const string frameStr = string(frameName + to_string(frame) + ".ppm");
