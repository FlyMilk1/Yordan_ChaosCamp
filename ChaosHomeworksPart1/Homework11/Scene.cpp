    ...
    if (!materialsVal.IsNull() && materialsVal.IsArray()) {
        for (int matId = 0; matId < materialsVal.Size(); ++matId) {
            ...
            if (matType == "refractive") {
                ...
                Material tempMat(0, smoothShadingVal.GetBool(), matType, ior);
                ...
                Material tempMat(textureId, smoothShadingVal.GetBool(), matType);
                materials.push_back(tempMat);
    ...
    if (!objectsVal.IsNull() && objectsVal.IsArray()) {
        for (int objId = 0; objId < objectsVal.Size(); ++objId) {
            ...
            geometryObjects.push_back(
                loadMesh(verticesVal.GetArray(), indicesVal.GetArray(), uvsVal.GetArray(), materials[matIdVal.GetInt()])
            );
        }
    }
    //Lights
...
Mesh Scene::loadMesh(const rapidjson::Value::ConstArray& arrayVertices, const rapidjson::Value::ConstArray& arrayIndices,  const rapidjson::Value::ConstArray& arrayUVs, const Material& material) {
    ...
    std::vector<vec3> uvs;
    for (int uv = 0; uv <= arrayUVs.Size() - 3; uv += 3) {
        uvs.push_back({
            static_cast<float>(arrayUVs[uv].GetDouble()),
            static_cast<float>(arrayUVs[uv + 1].GetDouble()),
            static_cast<float>(arrayUVs[uv + 2].GetDouble())
            });
    }
