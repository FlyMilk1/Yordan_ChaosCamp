...
//Raytracing of triangles with camera movement, animation, lightning and scene representation
#include <fstream>
#include <cmath>
#include <cstdlib>
#include <vector>
...
class Material{
	public:
		Material(const vec3& albedo={0,0,0}) : albedo(albedo){};
	...
	public:
		void addTriangles(std::vector<triangle>& triangleArray) {
			for(int firstIndex = 0; firstIndex <= triangleVertIndices.size()-2; firstIndex += 3) {
				Material triangleMat ({0.8,0,1.0});
...
vec3 shade(const vec3& p, const Scene& scene, const triangle& checkedTriangle, const triangle * triangleArray, const int triangleCount){
	const std::vector<Light>& lights = scene.lights;
	float result=0;
	for(int lightId=0; lightId<lights.size(); lightId++){
		vec3 lightDir = subtractVec3(lights[lightId].getPosition(), p);
		float sr = length(lightDir);
		normalizeVector(lightDir);
		float cosLaw = std::max(0.0f, dot(checkedTriangle.normalVec,lightDir));
		float sa = 4*pi*sr*sr;
		Ray shadowRay = {(p+(checkedTriangle.normalVec*SHADOW_BIAS)), lightDir};
...
Color rayIntersect(const triangle * triangleArray, const int triangleCount, const Ray& ray, const Scene& scene){
	...
	for (int triangleId = 0; triangleId < triangleCount; triangleId++) {
		...
		float t = dot(n, subtractVec3(tri.v0, ray.origin)) / rProj;
		if (t <= 0) continue;

		vec3 p = ray.origin + ray.dir * t;

		
		vec3 e0 = subtractVec3(tri.v1, tri.v0);
		vec3 e1 = subtractVec3(tri.v2, tri.v1);
		vec3 e2 = subtractVec3(tri.v0, tri.v2);

		if (
			dot(n, cross(e0, subtractVec3(p, tri.v0))) >= 0 &&
			dot(n, cross(e1, subtractVec3(p, tri.v1))) >= 0 &&
			dot(n, cross(e2, subtractVec3(p, tri.v2))) >= 0
		) {
	...
	if (closestTriangle) {
		vec3 shaded = shade(closestPoint, scene, *closestTriangle, triangleArray, triangleCount);
