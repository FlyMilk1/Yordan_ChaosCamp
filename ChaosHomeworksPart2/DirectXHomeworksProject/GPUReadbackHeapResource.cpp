...
GPUReadbackHeapResource::GPUReadbackHeapResource(ID3D12Device* device, const DXResource* RenderTargetResource)
{
	if (!device || !RenderTargetResource) return;
...
const D3D12_PLACED_SUBRESOURCE_FOOTPRINT GPUReadbackHeapResource::getPlacedFootprint() const
{
