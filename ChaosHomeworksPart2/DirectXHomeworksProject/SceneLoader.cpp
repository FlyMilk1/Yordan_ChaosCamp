...
bool SceneLoader::loadScene(const std::string& filePath, Scene* scene)
{
    ...
    rapidjson::Document doc = loadDocument(filePath);
    ...
    if (!objectsVal.IsNull() && objectsVal.IsArray()) {
        std::vector<SceneObject*> geometryObjects;
        for (int objId = 0; objId < objectsVal.Size(); ++objId) {
            ...
            if (originVal.IsArray()) {
                origin = loadFloat3(originVal.GetArray());
            }

            GeometrySceneObject* geoObject = new GeometrySceneObject(
                loadMesh(verticesVal.GetArray(), indicesVal.GetArray(), uvsVal),
                origin
            );
			geometryObjects.push_back(geoObject);
        }
...
DirectX::XMFLOAT3 SceneLoader::loadFloat3(const rapidjson::Value::ConstArray& array) {
    assert(array.Size() == 3);
...
Mesh SceneLoader::loadMesh(const rapidjson::Value::ConstArray& arrayVertices, const rapidjson::Value::ConstArray& arrayIndices, const rapidjson::Value& arrayUVs, const Material& material) {
 ...
 /*   std::vector<vec3> uvs;
    ...
    Mesh tempMesh;
	tempMesh.vertices.insert(tempMesh.vertices.begin(), vertices.begin(), vertices.end());
	tempMesh.indices.insert(tempMesh.indices.begin(), indices.begin(), indices.end());
